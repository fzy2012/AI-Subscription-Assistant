...
const
 IconBase = ({ children, className = "", size = 24, ...props }) => (
...
const
 RefreshCcw = (props) => <IconBase {...props}><path d="M3 2v6h6" /><path d="M3 13a9 9 0 1 0 3-7.7L3 8" /></IconBase>;
 ...
 KNOWLEDGE_BASE = {
  ...
  platforms: {
    'openai': {
      name: 'OpenAI (ChatGPT)',
      policy
: 'OpenAI 政策较严，但对欧盟/英国用户有14天冷静期。对于"技术故障"或"误操作"且未使用的申诉成功率较高。',

      refundUrl
: 'https://help.openai.com/en/articles/7232916-how-do-i-cancel-my-subscription'
...
'midjourney': {
      ...
      name: 'Midjourney',
      policy
: 'Midjourney 有著名的"零使用退款"政策：如果续费后 GPU 分钟数使用量为 0，通常可自动或人工全额退款。',
...
'apple': {
      ...
      policy
: 'Apple 掌握最终裁量权。对于"儿童误操作"或"应用无法使用"的理由，通过 reportaproblem.apple.com 申诉成功率极高。',
...
: <Zap className="text-amber-500" />,
    ...
    title: '误触 / 冲动消费',
    description
: '手滑点了订阅，或者原本只想试用却扣了年费。',

    emailSubject
: "Refund Request - Accidental Subscription",

    defaultTemplate
: (data) => `I am writing to request a refund for a subscription that was purchased by mistake on ${data.date}. 


I realized the error immediately and have not intended to use the premium features. As this was an accidental transaction, I kindly request a cancellation and full reversal of the charge of 
${data.amount}.`
...
: '试用期结束忘记关自动续费，完全没用过。',
    ...
    defaultTemplate
: (data) => `My subscription auto-renewed on ${data.date} contrary to my intent. I meant to cancel before the renewal date but missed the window.
...
: '付了钱但功能报错、宕机或无法登录。',
    ...
    defaultTemplate
: (data) => `I am requesting a refund because the service has been unusable for me. Since subscribing on ${data.date}, I have encountered persistent technical issues: [${data.errorDescription || "Login/Generation errors"}].
...
: '效果不满意',
    ...
    emailSubject
: "Refund Request - Service Not As Described",

    defaultTemplate
: (data) => `I recently subscribed to your service but found the quality significantly different from what was advertised. Specifically: [${data.issueDetails || "The output quality is poor"}].
...
${data.issueDetails || "Please explain your situation here"}].
...
: { type: "STRING" },
        ...
        date: { type: "STRING" },
        usageStatus
: { type: "STRING", enum: ["zero", "low", "high"] },
...
...prev,
        ...
        ...result,
        userStory
: formData.userStory // keep the story
...
={formData.userStory}
              ...
              placeholder
="例如：昨天我不小心点了 Midjourney 的年费订阅，扣了我 200 美元，但我一次都没用过，想退款..."
...
<button 
              ...
              className
="bg-blue-600 text-white px-6 rounded-xl font-bold text-sm hover:bg-blue-700 transition-colors disabled:opacity-50 flex items-center gap-2 shrink-0 w-32 justify-center"
...
={scenario.id}
            ...
            className
={`group relative p-6 bg-white border rounded-2xl cursor-pointer transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${scenario.color}`}
...
={handleInputChange}
                    ...
                    placeholder="20.00"
                    className
="w-full pl-7 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none transition-all font-medium text-gray-900"
...
={handleInputChange}
                  ...
                  className
="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none transition-all font-medium text-gray-900"
...
<label className="block text-sm font-bold text-gray-700 mb-2">
                  ...
                  {formData.scenario === 'technical' ? '故障描述' : 
                   formData
.scenario === 'dissatisfied' ? '不满之处' : '情况说明'}
...
<textarea 
                  ...
                  name
={formData.scenario === 'technical' ? 'errorDescription' : 'issueDetails'}
...
={handleInputChange}
                  ...
                  className
="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none transition-all font-medium text-gray-900 h-28 resize-none"
...
<button 
                      ...
                      onClick
={() => setFormData({...formData, usageStatus: status})}
...
={() => setStep(3)}
              ...
              disabled
={!formData.platform || !formData.amount}

              className
="w-full mt-4 bg-gray-900 hover:bg-black disabled:bg-gray-300 text-white font-bold py-4 rounded-xl shadow-lg transition-all transform active:scale-[0.98] flex items-center justify-center gap-2"
...
{step === 1 && <TemplateSelectView />}

        
{step === 2 && <DetailsFormView />}
